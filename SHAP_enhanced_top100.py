...
def main():
    ...
    output_dir = os.path.join("5_outputs_maps", "SHAP_top100")
    ensure_dir_exists(output_dir)  # Ensure that the directory is created if not already existing

    model, full_dataset, X, labels_df = load_data_and_model()
    
    # Sort the labels DataFrame by Predicted_Prob in descending order to get the top 100 Grid_IDs
    labels_df_sorted = labels_df.sort_values(by="Predicted_Prob", ascending=False)
    
    # Select the top 100 Grid_IDs based on highest Predicted_Prob
    top_100_labels_df = labels_df_sorted.head(100)
    
    # Filter the full dataset to include only the top 100 Grid_IDs
    filtered_dataset = full_dataset[full_dataset['Grid_ID'].isin(top_100_labels_df['Grid_ID'])]
    ...
    lookup_dict = {
        ...
        } for _, row in top_100_labels_df.iterrows()
    }
    ...
    grid_ids = list(grid_indices.keys())  # Now only contains the top 100 Grid_IDs
    ...
    output_pdf_path = os.path.join("5_outputs_maps/SHAP_top100", "SHAP_Explainability_Summary_top100.pdf")
    
    # Ensure the directory for the output PDF exists
    ensure_dir_exists(os.path.dirname(output_pdf_path))  # Ensure the parent directory exists
